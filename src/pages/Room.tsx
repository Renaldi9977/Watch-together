...
export default function Room() {  const { roomId } = useParams<{ roomId: string }>();
  ...
  const saveSticker = (dataUrl: string) => {    const newStickers = [...customStickers, dataUrl];
  ...
  useEffect(() => {
    ...
    socket.on("user-joined", (user) => {
      setParticipants((prev) => {
        if (prev.find((p) => p.id === user.id)) return prev;        return [...prev, user];
      });
  ...
  const setupVoiceDetection = (userId: string, stream: MediaStream) => {
    ...
    analyser.smoothingTimeConstant = 0.8;    const source = audioCtx.createMediaStreamSource(stream);
  ...
  const connectAudioStream = (userId: string, stream: MediaStream) => {
    ...
    // Explicitly try playing to catch and handle autoplay policies    const playPromise = audioEl.play();
  ...
  const toggleMic = async () => {
    if (micEnabled) {
      ...
      try {
        ...
        localStreamRef.current = stream;        setMicEnabled(true);
  ...
  const handleLoadVideo = () => {
    ...
    setVideoUrl(inputUrl);    // Simulate loading feedback
  ...
  const handleYouTubeReady = (e: any) => {    playerRef.current = e.target;
  ...
  const handleSendChat = (
    ...
    if (chatInputRef.current && !customText) {
      chatInputRef.current.value = "";      chatInputRef.current.style.height = "auto";
    }
    setReplyingTo(null);
  };
      ...
      <div className="flex flex-col flex-1 overflow-hidden md:grid md:grid-cols-12 md:gap-4">
        ...
        <div className="md:col-span-8 flex flex-col md:bg-white/5 md:border md:border-white/10 md:rounded-3xl shrink-0 md:h-full w-full relative">
            ...
            {useMemo(() => videoUrl ? (
              <div className="w-full h-full pointer-events-auto transition-opacity duration-1000 opacity-100 flex items-center justify-center">
                {!extractYouTubeId(videoUrl) ? (
                  ...
                  {/* ✅ YOUTUBE COMPONENT YANG SUDAH DIPERBAIKI */}
                  <YouTube
                    ...
                    opts={{
                      ...
                      playerVars: {
                        ...
                        modestbranding: 1,
                        rel: 0,
                        enablejsapi: 1,  // ← ✅ DITAMBAHKAN: Wajib untuk API control
                        // origin dihapus agar bekerja di iframe environment
                      },
                    }}
                    ...
                    iframeStyle={{ border: 'none' }}  // ← ✅ Optional: hapus border default
                  />
                )}
                {!isHost && (
                  <div
          ...
          <div className="flex flex-col p-3 md:p-5 gap-4 shrink-0 transition-all z-20 md:border-none border-t border-white/5">
            <div className="bg-[#161c2d] border border-[#2e3c5a] shadow-lg rounded-[24px] p-4 flex flex-col gap-4">              <div className="flex flex-row gap-3 items-center w-full">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-4 overflow-x-auto py-3 px-2 -mx-2 no-scrollbar items-center">
                      ...
                      >
                        ...
                        >
                          <div
                            ...
                            className={cn(
                              "avatar cursor-pointer w-14 h-14 md:w-16 md:h-16 rounded-full border-[3px] transition-all duration-300 flex items-center justify-center bg-[#0a0f1a] relative z-10",                              selectedParticipant === p.id
                  ...
                  }), [participants, speakingUsers, selectedParticipant, isHost, hostId])}
                ...
                <div className="md:hidden flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-[12px] border border-white/10">
                  <div
                    className={cn(
                      ...
                      socketRef.current?.connected
                        ? "bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.6)]"                        : "bg-red-500 animate-pulse",
                    )}
                  />
            ...
            <div className="flex-1 relative flex flex-col overflow-hidden w-full bg-[#090d14] md:bg-transparent">
              ...
              <div className="flex-1 overflow-y-auto overflow-x-hidden relative z-10 w-full">
                <div className="px-5 py-4 md:px-8 md:py-6 relative z-10 min-h-full">
                  {useMemo(() => messages.map((msg, index) => {
                    ...
                    const avatarContent = msg.avatar ? (
                      ...
                      >
                        <img
                          src={msg.avatar}
                          alt={msg.sender}                          className="w-full h-full rounded-full object-cover bg-black/50 aspect-square"
                        />
                      </div>
                    ) : (
                      <div 
                          ...
                          <div
                            onContextMenu={(e) => {
                              ...
                              setReplyingTo({
                                id: msg.id,
                                sender: msg.sender,
                                text: msg.isSticker ? "Sticker" : msg.text || "Photo",
                            ...
                            onDoubleClick={() =>
                              setReplyingTo({
                                id: msg.id,
                                sender: msg.sender,
                                text: msg.isSticker ? "Sticker" : msg.text || "Photo",                                imageUrl: msg.imageUrl,
                              })
                            }
                            className={cn(
                          ...
                          >
                            {msg.replyTo && (
                              <div
                                className={cn(
                                  ...
                                  msg.isSticker && !msg.text
                                    ? "rounded-lg bg-black/40 border-slate-500"
                                    : "rounded-md bg-black/20",
                                )}
                                style={(!msg.isSticker || msg.text) ? { borderColor: bubbleFrameColor } : {}}
                              >
                                <div className="flex flex-col truncate flex-1 min-w-0">
                                  <span className="text-[10.5px] font-bold text-cyan-400 leading-tight">
                                    {msg.replyTo.sender}
                                  </span>
                                  <span className="text-[12px] text-slate-300 truncate h-[16px] overflow-hidden leading-tight mt-0.5">
                                    {msg.replyTo.text}
                                  </span>
                                </div>
                                {msg.replyTo.imageUrl && (
                                  <img
                                    ...
                                    className="w-7 h-7 object-cover rounded-[4px] opacity-80 shrink-0 ml-1"
                                    alt="reply-thumb"
                                  />
                                )}
                              </div>
                            ...
                            <div className={cn("flex flex-col w-full max-w-full", isMe ? "items-end" : "items-start")}>
                              ...
                              {msg.text && (
                                <div className={cn("flex items-end gap-2 flex-wrap", isMe ? "justify-end" : "justify-start")}>
                                  <p className="text-[14.5px] leading-[1.4] whitespace-pre-wrap break-words font-[400]">
                                    {msg.text}
                                  </p>
                                  <div className="flex items-center justify-end gap-0.5 shrink-0 h-4 mt-1 opacity-70">
                                    <span className="text-[10px] font-mono leading-none pt-1">
                                      {msg.time}
                                    </span>
                                    {isMe && (
                                      <CheckCheck className="w-[14px] h-[14px] text-cyan-400 ml-0.5 pt-0.5" />
                                    )}
                                  </div>
                                </div>
                              )}
                            ...
                            {!msg.text && (
                              ...
                              >
                                ...
                                {isMe && (
                                  <CheckCheck className="w-[14px] h-[14px] text-cyan-400" />
                                )}
                              </div>
                            )}
                          </div>
                  ...
                  );}), [messages, profile?.name, profile?.frameColor])}                  <div ref={chatEndRef} />
                </div>
              </div>
                        ...
                        {customStickers.map((sticker, idx) => (
                          ...
                          >
                            <img
                              ...
                              onClick={() => {                                handleSendChat(undefined, "", sticker, true);
                ...
                >
                  <div className="flex-1 bg-[#1e293b]/70 backdrop-blur-md border border-slate-700/50 rounded-[24px] flex items-center pr-2 pl-1 relative z-10 transition-colors focus-within:border-cyan-500/40 shadow-sm">
                    <button
                      ...
                      className={cn(
                        "p-2 text-slate-400 hover:text-white shrink-0 active:scale-95 transition-all",                        showStickers && "text-cyan-400",
                      )}
                    >
        ...
        {previewWallpaper && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-[#0f1521] border border-[#2e3c5a] rounded-2xl w-full max-w-sm overflow-hidden flex flex-col shadow-2xl relative">              {/* Preview Area */}
